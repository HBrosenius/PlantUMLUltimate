...
export interface ActivityNoteInput { text: string; placement: "left" | "right" | "top" | "bottom"; color?: string; targetId?: string; floating?: boolean }
...
export interface ActivityArrowInput { label?: string; color?: string; lineStyle?: "solid" | "dashed" | "dotted" | "bold"; targetId?: string }
...
export function moveActivityPartition(source: string, document: ActivityDocument, item: ActivityPartition, parentId?: string) {
  ...
  if (parentId && (!parent || parent.id === item.id || (parent.sourceRange.from > item.sourceRange.from && parent.sourceRange.to < item.sourceRange.to))) return source;
...
const noteLine = (value: ActivityNoteInput) => `${value.floating ? "floating " : ""}note ${value.placement}${value.color ? ` ${value.color.startsWith("#") ? value.color : `#${value.color}`}` : ""}\n${value.text.trim()}\nend note`;
...
export function moveActivityActionToPartition(source: string, document: ActivityDocument, item: ActivityNode, partitionId?: string) {
...
export function updateActivityNoteWithTarget(source: string, document: ActivityDocument, note: ActivityNote, value: ActivityNoteInput) {
...
export function reorderActivityAction(
  ...
  if (item.id === target.id || item.kind !== "action" || target.kind !== "action")
    return source;
  ...
  if (item.partitionId === target.partitionId && document.controls.some((control) => control.sourceRange.from > first && control.sourceRange.from < last)) return source;
