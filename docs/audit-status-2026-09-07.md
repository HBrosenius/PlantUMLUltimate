...
- Pull request [#47](https://github.com/HBrosenius/PlantUMLUltimate/pull/47) merged on
  2026-09-11 after the protected CI matrix passed. It covered validation, PWA, CodeQL,
  collaboration, and Chromium/Firefox/WebKit browser shards for the single-file project
  workflow.
...
| S4 — same-origin unsandboxed renderer                      | **Deferred; engine constraint confirmed** | Source filtering removes remote include/import/theme directives and SVG sanitization forbids `<style>`, but `use-renderer.ts` still creates an unsandboxed `srcdoc` frame. Opaque-frame prototypes failed across the PWA/browser matrix because the static host and service worker cannot reliably deliver the large renderer modules into that origin. A dedicated Web Worker prototype was also rejected: the bundled `@plantuml/core` documentation states that its TeaVM engine requires a real DOM, and testing reproduced DOM/SVG measurement failures and stalled Graphviz rendering. All prototypes were reverted rather than shipping broken previews. | Keep the existing local iframe renderer for now. Revisit isolation only with a separate renderer origin/process or a headless, Worker-compatible upstream engine. Any future prototype must preserve offline rendering for all six diagram types and pass Chromium, Firefox, WebKit, and PWA tests before merge. |
...
| S7 — Jira refresh/decrypt failure blocks disconnect        | **Fixed in source**                       | Invalid/dead sessions are converted to a 401 and deleted; disconnect is dispatched without requiring token refresh. OAuth denial redirects back to the app. Worker tests cover invalid encrypted sessions, refresh rejection, disconnect, and denial. Controlled browser tests cover expired authorization, OAuth denial, transient field-loading failure, and disconnect availability.                                                                                                                                                                                                                                                                         | Add a concurrent refresh regression test and run an authenticated staging smoke check. Keep disconnect available in every failure state.                                                                                                                                                                         |
