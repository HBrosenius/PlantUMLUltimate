...
| S4 — same-origin unsandboxed renderer                      | **Still open; constraint reproduced** | Source filtering removes remote include/import/theme directives and SVG sanitization forbids `<style>`, but `use-renderer.ts` still creates an unsandboxed `srcdoc` frame. A 2026-09-11 prototype using `sandbox="allow-scripts"`, opaque-origin messaging, and the existing per-frame channel prevented the local bundled renderer modules from loading; the static host does not provide cross-origin module access to an opaque frame. The prototype was reverted rather than shipping broken previews. | Serve renderer assets from an isolation-compatible origin with the required module policy, or bundle them into a sandbox-compatible frame. Then repeat the opaque-origin handshake prototype, recreate the frame after timeouts, and test stale responses plus offline rendering for all six diagram types. Confirm possible network-producing syntax with an instrumented frame before adding more filters. |
