...
`@plantuml-studio/diagram-wbs` models hierarchical work-breakdown nodes, diagnostics, completions, and source-preserving subtree operations. The web application resolves its parser through the configured registry and routes preview subtree moves plus primary node and relationship commands through the adapter's typed visual-operation API. Settings rewriting and semantic-symbol rename support remain specialized application paths. The preview uses the canonical PlantUML SVG with a semantic node interaction layer; the hierarchy model is derived from source and is never persisted separately.
