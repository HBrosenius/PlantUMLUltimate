...
export function VerticalSeparatorInspector({
  separator,
  tasks,
  onApply,
  onDelete,
      ...
      >
        <label>
          ...
          <select
            ...
            onChange={(event) => setValue((current) => ({ ...current, taskLabel: event.target.value }))}
          >
            {tasks.map((task) => (
        ...
        <label>
          ...
          <select
            value={value.anchor}
            onChange={(event) => setValue((current) => ({ ...current, anchor: event.target.value as "start" | "end" }))}
          >
        ...
        <label>
          ...
          <input
            ...
            step="1"
            value={value.offset}
            onChange={(event) => setValue((current) => ({ ...current, offset: Number(event.target.value) }))}
          />
        </label>
        {value.offset > 0 && (
          <label>
            ...
            <select
              ...
              onChange={(event) =>
                setValue((current) => ({ ...current, direction: event.target.value as "after" | "before" }))
              }
            >
