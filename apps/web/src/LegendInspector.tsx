        ...
        {value.length ? (
          value.map((entry, index) => (
            <label key={entry.color}>
              ...
              <input
                ref={entry.color.toLowerCase() === focusColor?.toLowerCase() ? focusedInput : undefined}
                aria-label={`Legend label for ${entry.color}`}
                value={entry.label}
                onChange={(event) =>
                  setValue((current) =>
          ...
          <p className="inspector-note">Assign a color to an item to create the legend.</p>
        )}
