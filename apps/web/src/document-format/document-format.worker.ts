...
self.addEventListener("message", (event: MessageEvent<Record<string, unknown>>) => {
  ...
  const operation = request.operation === "encode"
    ? encodeDocument(request.document as Parameters<typeof encodeDocument>[0], {
        ...(request.options as Parameters<typeof encodeDocument>[1]), signal: controller.signal,
      })
    : decodeDocument(request.bytes as Uint8Array, {
        ...(request.options as Parameters<typeof decodeDocument>[1]), signal: controller.signal,
      });
