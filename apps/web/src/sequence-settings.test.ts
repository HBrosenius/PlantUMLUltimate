...
describe("Sequence settings", () => {
  it("parses presentation, visibility, activation, and numbering settings", () => {
    expect(
      ...
      autonumber: true,
      autonumberStart: "10",
      autonumberIncrement: "5",
      autonumberFormat: "000",
    });
  });
  ...
  it("updates managed settings while preserving messages and flow numbering controls", () => {
    const source =
      "@startuml\ntitle Old\nautonumber 1\nA -> B: Request\nautonumber stop\nA --> B: Quiet\nautonumber resume\n@enduml";
    const updated = updateSequenceSettings(source, {
      ...parseSequenceSettings(source),
      title: "New",
      hideFootbox: true,
      autonumberStart: "10",
      autonumberIncrement: "2",
      autonumberFormat: "00",
    });
  ...
  it("round-trips the Sequence visual palette", () => {
    ...
    expect(updateSequenceSettings(source, { ...parsed, arrowColor: "#DC2626" })).toContain(
