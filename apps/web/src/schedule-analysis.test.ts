...
import { baselineBarGeometry, calculateTaskVariance, criticalPathTaskIds, timelineBaselineX } from "./schedule-analysis";
...
@endgantt`).document;
  ...
  it("reports movement against resolved baseline dates", () => {
    ...
    expect(calculateTaskVariance(current, baseline)).toEqual([{ taskId: "a", startDays: 2, endDays: 2 }]);
  });

  it("resolves a stored baseline independently", () => {
    const source = "@startgantt\nProject starts 2026-09-01\n[A] lasts 2 days\n@endgantt";
    const document = parseGantt(source).document;
    expect(resolveTaskDates(document.tasks, document.dependencies, "2026-09-01", parseGanttCalendar(source)).get("a")?.end).toBe("2026-09-02");
  });
  ...
  it("anchors the ghost to its baseline timeline column instead of the current task", () => {
    const columns = [{ date: "2026-09-01", x: 5.22 }, { date: "2026-09-02", x: 21.22 }];
