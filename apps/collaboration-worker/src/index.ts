...
function connectionFrom(value: unknown, fallbackId: string): ConnectionAttachment {
  if (value && typeof value === "object" && "participant" in value) {
    const candidate = value as Partial<ConnectionAttachment>;
    return {
      participant: participantFrom(candidate.participant, fallbackId)!,
      role: candidate.role === "viewer" ? "viewer" : "editor",
      owner: candidate.owner === true,
    };
  }
  return { participant: participantFrom(value, fallbackId)!, role: "editor", owner: false };
}

export class CollaborationRoom extends DurableObject<Env> {
  private document = new Y.Doc();
  private ownerToken: string | undefined;
  private editorToken: string | undefined;
  ...
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    ctx.blockConcurrencyWhile(async () => {
      ...
      `);
      const accessColumns = this.ctx.storage.sql
        .exec<{ name: string }>("PRAGMA table_info(room_access)")
        .toArray()
        .map((column) => column.name);
      if (!accessColumns.includes("editor_token"))
        this.ctx.storage.sql.exec("ALTER TABLE room_access ADD COLUMN editor_token TEXT");
      if (!accessColumns.includes("viewer_token"))
        this.ctx.storage.sql.exec("ALTER TABLE room_access ADD COLUMN viewer_token TEXT");
      const access = this.ctx.storage.sql
        .exec<{ owner_token: string; editor_token: string | null; viewer_token: string | null; revoked: number }>(
          "SELECT owner_token, editor_token, viewer_token, revoked FROM room_access WHERE singleton = 1",
        )
        .toArray()[0];
      this.ownerToken = access?.owner_token;
      this.editorToken = access?.editor_token ?? undefined;
      this.viewerToken = access?.viewer_token ?? undefined;
  ...
  private participants(): Participant[] {
    return this.ctx.getWebSockets().map((socket) => {
      ...
      return { ...connection.participant, role: connection.role };
    });
  }
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (
      !this.ownerToken &&
      validToken(requestedOwnerToken) &&
      validToken(requestedEditorToken) &&
      validToken(requestedViewerToken)
    ) {
      this.ownerToken = requestedOwnerToken;
      this.editorToken = requestedEditorToken;
      this.viewerToken = requestedViewerToken;
      this.ctx.storage.sql.exec(
        "INSERT INTO room_access (singleton, owner_token, editor_token, viewer_token, revoked) VALUES (1, ?, ?, ?, 0)",
        requestedOwnerToken,
    ...
    const role: CollaborationRole | undefined = owner
      ? "editor"
      : this.editorToken && accessToken === this.editorToken
        ? "editor"
    ...
    server.serializeAttachment({ participant, role, owner } satisfies ConnectionAttachment);
